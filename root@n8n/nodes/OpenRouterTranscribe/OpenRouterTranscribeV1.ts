		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (inputData === 'binary') {
					...
					if (audioFormat === 'auto') {
						if (!fileName) {
							throw new NodeOperationError(this.getNode(), 'Cannot auto-detect audio format: filename not available', {
								itemIndex: i,
							});
						}
					...
					if (audioFormat === 'auto') {
						throw new NodeOperationError(this.getNode(), 'Auto-detect format is not available for text input mode. Please select a specific audio format.', {
							itemIndex: i,
						});
					}
				}
